...
public class SugoApi {
    ...
    public SugoJobPreviewPageDto getJobPreviews(final Pageable pageable) {
        final ResponseEntity<SugoJobPreviewPageDto> response = get(JOB_ENDPOINT, pageable, SugoJobPreviewPageDto.class);
        if (response.getStatusCode() == HttpStatus.OK) {
            return Objects.requireNonNull(response.getBody());
        } else {
