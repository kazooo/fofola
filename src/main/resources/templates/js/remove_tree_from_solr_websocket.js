...
$(function () {
    ...
    var socket = new SockJS('/remove-websocket');
    ...
    $( "#remove_button" ).click(function() { sendUuids(); showReindexPanel(false); });
});
...
function sendUuids() {
    ...
    for(var i = 0; i < uuids.length; i++) {
        stompClient.send("/remove-websocket", {}, JSON.stringify(
            {'root_uuid': uuids[i], 'solr_host': $("#enter_host").val()}
        ));
    }
    clearTable();
    setWaiting(false);
...
function setWaiting(set) {
    showSpin(set);
    var table = document.getElementById('uuid_table');
    var reindex_button = document.getElementById('remove_button');
    if (set) {
