...
$(function () {
    ...
    var socket = new SockJS('/transfer-websocket');
    ...
    $( "#transfer_button" ).click(function() { sendUuids(); showReindexPanel(false); });
});
...
function sendUuids() {
    ...
    for(var i = 0; i < uuids.length; i++) {
        stompClient.send("/transfer-websocket", {}, JSON.stringify(
            {'root_uuid': uuids[i],
                'src_solr_host': $("#src_solr_host").val(),
                'dst_solr_host': $("#dst_solr_host").val()
            }
        ));
    }
    clearTable();
...
function setWaiting(set) {
    showSpin(set);
    var table = document.getElementById('uuid_table');
    var reindex_button = document.getElementById('transfer_button');
    if (set) {
