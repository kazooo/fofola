...
import {addOneUuidInfo, addUuidInfo, createActionType} from "./slice";
...
const GET_ONE_UUID_INFO = createActionType("GET_ONE_UUID_INFO");

export const getOneUuidInfo = createAction(GET_ONE_UUID_INFO);
...
export default function* watcherSaga() {
    yield takeEvery(GET_ONE_UUID_INFO, getOneUuidInfoSaga);
...
function* getOneUuidInfoSaga(action) {
    try {
        const payload = yield call(() => request
            .get("/uuid-info/" + action.payload)
        );
        yield put(addOneUuidInfo(payload.body));
    } catch (e) {
